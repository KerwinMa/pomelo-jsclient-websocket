(function () {
    ...
    var native_protobuf = null;
    var native_encoder = null;
    var native_decoder = null;

    if (typeof Protobuf === "function") {
        native_protobuf = Protobuf;
    }
    else {
        try {
            decodeIO_protobuf = require('pomelo-decodeIO-protobuf');
        }
        catch (err) {
    ...
    var defaultEncode = pomelo.encode = function (reqId, route, msg) {
        ...
        } else if (native_encoder && native_encoder.lookup(escapedRoute)) {
            msg = new Uint8Array(native_encoder.serialize(msg, escapedRoute));
        } else if (decodeIO_encoder && decodeIO_encoder.lookup(escapedRoute)) {
            var Builder = decodeIO_encoder.build(escapedRoute);
            msg = new Uint8Array(new Builder(msg).encodeAB());
        } else {
    ...
    var connect = function (params, url, cb) {
        ...
        if (root.localStorage && root.localStorage.getItem('protos') && protoVersion === 0) {
            ...
            if (!!native_protobuf) {
                native_encoder = new native_protobuf();
                native_encoder.load(base64.base64DecToArr(clientProtos.desc).buffer);
                native_decoder = new native_protobuf();
                native_decoder.load(base64.base64DecToArr(serverProtos.desc).buffer);
            }
        }
        //Set protoversion
    ...
    pomelo.disconnect = function (voluntary) {
        if (socket) {
    ...
    var reset = pomelo.stopReconnec = function () {
        reconnect = false;
    ...
    var processMessage = function (pomelo, msg) {
        if (msg.id) {
            ...
            pomelo.emit(msg.route, msg.body); // server push message
        }
    };
    ...
    var deCompose = function (msg) {
        ...
        } else if (native_decoder && native_decoder.lookup(escapedRoute)) {
            return native_decoder.parse(msg.body, escapedRoute);
